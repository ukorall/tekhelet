...
object GadilBuilder {
    ...
    private fun totalWinds(c: KnotComposition): Int {
        ...
        val chulyot = when (c.chulyotCount) {
            ...
            // השיטה הזו לא מכריעה בין 7 ל-13, ולכן צריך לבחור מה **לצייר**.
            // בחרתי ב-13 כי זו האפשרות המלאה, ומי שירצה 7 רואה תת-קבוצה של מה
            // שמצויר. אבל הבחירה הזו לא נשארת חבויה בקוד: ההוראות אומרות
            // במפורש שהשיטה משאירה את זה פתוח, וההמלצה על מה לקנות מציינת
            // שאם בוחרים 7 אפשר להסתפק בסט קצר וזול יותר.
